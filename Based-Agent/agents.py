...
def transfer_asset(amount, asset_id, destination_address):
    ...
    try:
        ...
        if asset_id.lower() in ["eth", "usdc"]:
            ...
            return (
                f"Transferred {amount} {asset_id}{gasless_msg} to {destination_address}"
            )
...
def swap_assets(
    amount: Union[int, float, Decimal], from_asset_id: str, to_asset_id: str
):
    """
...
def create_register_contract_method_args(
    base_name: str, address_id: str, is_mainnet: bool
) -> dict:
    """
    ...
    try:
        ...
        address_data = resolver_contract.encode_abi(
            "setAddr", args=[name_hash, address_id]
        )
        ...
        raise ValueError(
            f"Error creating registration arguments for {base_name}: {str(e)}"
        )
